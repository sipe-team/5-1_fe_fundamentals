...
- [ ] 날짜 선택 (날짜를 변경하면 해당 날짜의 예약 현황을 조회)
- [ ] 회의실별 예약 현황을 타임라인(30분 단위 그리드)으로 표시
- [ ] 예약된 시간대와 빈 시간대를 시각적으로 구분
- [ ] 예약 블록에 제목 표시
- [ ] 예약 블록 클릭 시 예약 상세 페이지(`/reservations/:id`)로 이동
- [ ] 빈 시간대 클릭 시 예약 생성 페이지(`/reservations/new`)로 이동 (회의실, 날짜, 시간 정보를 적절한 방식으로 전달)
